...
[+] -------------------------------- ] | [ S E L F B O T - {aniketplyzx.user.name}  ] | [ ------------------------------------ [+]
""",
                1,
            )
        )
...
async def nitro(ctx):
    try:
        ...
        print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}{Fore.RED}[!] ERROR: {str(e)}")
...
async def stopactivity(ctx):
    ...
    print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}{Fore.GREEN}[+] ACTIVITY SUCCESFULLY STOPED⚠️ ")
...
async def wizz(ctx):
    ...
    if isinstance(ctx.message.channel, discord.TextChannel):
        ...
        print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}{Fore.GREEN}[+] WIZZING SUCCESSFUL✅ ")
...
async def spam(ctx, *, message):
    ...
    while True:
        try:
            ...
            print(f'{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}{Fore.RED}[!] ERROR : {e}')
            print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty} {Fore.GREEN}[+] SPAM SUCCESSFUL✅")
...
async def imgsearch(ctx, *, search=''):
    ...
    gis = GoogleImagesSearch('YOUR_API_KEY', 'YOUR_CX_CODE')
    search_params = {
        'q': search,
        'num': 1,
    }
    ...
    try:
        ...
        if gis.results():
            first_img_url = gis.results()[0].url
            embed = discord.Embed(title="Search's For:", description=f"`{search}`", color=0x00ff00)
            embed.add_field(name="Image URL:", value=f"[{first_img_url}]({first_img_url})")
...
async def banner(ctx):
    ...
    print(f"{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}{Fore.GREEN}[+] MAREACT  SUCCESSFUL✅ ")
...
async def nukezzz(ctx):
    ...
    channel_name = 'aniketplyzop'

    print(f'{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}{Fore.RED}[!] {Fore.BLUE}DELETING CHNNELS')
    ...
    print(f'{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}{Fore.GREEN}[!]{Fore.BLUE} CREATING CHNNELS')
    for i in range(18):
        try:
    ...
    print(f'{reset}[ {cyan}{time_rn}{reset} ] {gray}({green}+{gray}) {pretty}{Fore.GREEN}{Fore.RED}[!] {Fore.BLUE}BANNING ALL !')
    ...
    message_text = 'ANIKET OP JOIN : discord.gg/topsellers ||@everyone||'
